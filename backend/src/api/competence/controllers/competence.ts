...
export default factories.createCoreController('api::competence.competence', ({ strapi }) => ({
  ...
  async findOne(ctx) {
    ...
    const isOwner = Boolean(
      ctx.state.user &&
        (await isOwnChildEntry(strapi, 'api::competence.competence', id, ctx.state.user.id))
    );
  ...
  async update(ctx) {
    ...
    if (rawData.statut !== undefined) {
      const current = await strapi.db.query('api::competence.competence').findOne({ where: { documentId: id }, select: ['statut'] });
